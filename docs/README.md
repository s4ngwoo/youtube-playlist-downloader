# Documentation

Project docs for **YouTube Playlist & Audio Downloader** (open source, GPL-3.0).

**Languages:** [English](README.md) · [한국어](ko/README.md)

## English

| Document | Description |
| :--- | :--- |
| [PRIVACY.md](PRIVACY.md) | Privacy policy — local-only data handling |
| [TERMS.md](TERMS.md) | Terms of use and legal responsibility |
| [SECURITY.md](SECURITY.md) | Vulnerability reporting |
| [CONTRIBUTING.md](CONTRIBUTING.md) | How to contribute |
| [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) | Community standards |
| [FAQ.md](FAQ.md) | Frequently asked questions |
| [CONCURRENCY.md](CONCURRENCY.md) | Concurrent downloads — what it means and recommended values |
| [FILENAMES.md](FILENAMES.md) | Output names — title only, no-overwrites |
| [SUPPORT.md](SUPPORT.md) | Getting help |
| [CHANGELOG.md](CHANGELOG.md) | Version history |
| [RELEASING.md](RELEASING.md) | Tag releases and yt-dlp / FFmpeg sidecar policy |
| [THIRD_PARTY.md](THIRD_PARTY.md) | Bundled yt-dlp + LGPL FFmpeg notices |

## Korean (`docs/ko/`)

| Document | Description |
| :--- | :--- |
| [PRIVACY.md](ko/PRIVACY.md) | 개인정보 처리방침 |
| [TERMS.md](ko/TERMS.md) | 이용 약관 |
| [SECURITY.md](ko/SECURITY.md) | 보안·취약점 신고 |
| [CONTRIBUTING.md](ko/CONTRIBUTING.md) | 기여 가이드 |
| [CODE_OF_CONDUCT.md](ko/CODE_OF_CONDUCT.md) | 행동 강령 |
| [FAQ.md](ko/FAQ.md) | 자주 묻는 질문 |
| [CONCURRENCY.md](ko/CONCURRENCY.md) | 동시 다운로드 — 의미·권장값 |
| [FILENAMES.md](ko/FILENAMES.md) | 출력 파일명 — 제목만, no-overwrites |
| [SUPPORT.md](ko/SUPPORT.md) | 지원 안내 |
| [CHANGELOG.md](ko/CHANGELOG.md) | 변경 이력 |
| [RELEASING.md](ko/RELEASING.md) | 태그 릴리즈·yt-dlp 사이드카 정책 |

## READMEs

- English (default): [../README.md](../README.md)
- Korean: [../README_KO.md](../README_KO.md)

## License

See [../LICENSE](../LICENSE) (GPL-3.0). Bundled FFmpeg is LGPL — [THIRD_PARTY.md](THIRD_PARTY.md). This project remains open source.
