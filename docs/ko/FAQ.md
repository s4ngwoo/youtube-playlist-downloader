...
**macOS**(Apple Silicon)와 **Windows x64**는 [GitHub Releases](https://github.com/s4ngwoo/youtube-playlist-downloader/releases/latest)에서 받을 수 있습니다. 이 빌드에는 FFmpeg가 포함됩니다. **Intel Mac·Linux 공식 설치 파일은 계획하지 않습니다**(GUI가 필요하면 소스 빌드 · 대부분 CLI `yt-dlp`로 충분).
