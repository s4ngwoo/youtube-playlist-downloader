  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-48 w-full bg-neutral-900 rounded-xl border border-neutral-800">
        <div className="text-neutral-400">{t("history.loading")}</div>
      </div>
    );
  }
    ...
    <div className="w-full bg-neutral-900 rounded-xl border border-neutral-800 p-4 md:p-6 shadow-xl flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
        {historyItems.map((item) => (
          <div
            key={item.url}
            className="flex flex-col sm:flex-row gap-4 p-4 bg-neutral-950 rounded-lg border border-neutral-800 hover:border-neutral-700 transition-colors"
          >
            <div className="flex-1 min-w-0">
              <h3
                className="text-neutral-200 font-medium truncate"
                title={item.title || t("history.untitled")}
              >
