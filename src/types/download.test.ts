...
describe("isCancelledDownloadOutcome", () => {
  it("keeps an in-flight cancel even if the backend reports success", () => {
    expect(isCancelledDownloadOutcome("cancelled", "ok.download_complete")).toBe(true);
  });
  ...
  it("does not swallow completed or failed jobs", () => {
    expect(isCancelledDownloadOutcome("completed", "ok.download_complete")).toBe(false);
    expect(isCancelledDownloadOutcome("error", "error.all_failed")).toBe(false);
    expect(isCancelledDownloadOutcome("downloading", "ok.download_complete")).toBe(false);
  });
});
