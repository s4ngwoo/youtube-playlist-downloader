...
- **📱 모바일 호환 ZIP 내보내기 (NFC 정규화)**
  - macOS 고유의 NFD 파일명 인코딩 문제로 인해 Android 기기나 Windows로 파일 전송 시 한글이 깨지는 현상을 방지합니다. 
  - 앱 내에서 클릭 한 번으로 NFC 정규화가 적용된 모바일 호환 ZIP 파일을 생성할 수 있습니다.
